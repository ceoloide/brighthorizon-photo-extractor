...
export const Gallery: React.FC<GalleryProps> = ({ token, refreshTrigger }) => {
  ...
  return (
    <div className="space-y-4 font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
              ...
              <span className="text-xs bg-indigo-50 text-indigo-700 border border-indigo-100 px-2.5 py-0.5 rounded-full font-mono font-semibold">
      ...
      {/* Grid Display */}
      {loading ? (
        <div className="flex flex-col justify-center items-center py-20 text-slate-500 gap-3 bg-white rounded-2xl border border-slate-200 shadow-sm">
          <div className="w-7 h-7 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin" />
          <span className="text-xs font-medium text-slate-600">Loading photo gallery...</span>
        </div>
      ) : filteredList.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-200 text-slate-400 shadow-sm">
          ...
          <p className="text-xs text-slate-400 mt-1">Start an extraction job above to populate photos and videos.</p>
        </div>
      ) : (
          ...
          {filteredList.map((item) => {
            ...
            return (
              <div
                ...
                className="group relative bg-white rounded-2xl overflow-hidden border border-slate-200 hover:border-indigo-400 transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md flex flex-col"
              >
                <div className="aspect-square bg-slate-100 relative overflow-hidden flex items-center justify-center">
                  {isVideo ? (
                    <div className="w-full h-full flex flex-col items-center justify-center bg-slate-900 text-indigo-400">
                      <Video className="w-10 h-10 mb-1 opacity-90 group-hover:scale-105 transition-transform" />
                      <span className="text-[10px] font-mono font-bold uppercase bg-slate-800 text-slate-200 border border-slate-700 px-2 py-0.5 rounded-full">
                        VIDEO
                      </span>
                    </div>
                  ) : (
                ...
                <div className="p-3 text-xs space-y-1 bg-white border-t border-slate-100">
                  <div className="flex justify-between items-center text-slate-800 font-semibold truncate">
                    <span>{item.child}</span>
                    <span className="text-[10px] font-normal text-slate-500 flex items-center gap-1 font-mono">
      ...
      {activeItem && (
        <div className="fixed inset-0 z-50 bg-slate-900/75 flex items-center justify-center p-4">
          <div className="relative max-w-4xl w-full bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-white">
              <div>
                <h3 className="font-bold text-slate-900 text-sm truncate">{activeItem.original_filename}</h3>
                <p className="text-xs text-slate-500 mt-0.5">{activeItem.child} • {activeItem.date}</p>
              </div>
              <div className="flex items-center gap-2">
                <a
