...
user_data_dir = "/tmp/test_user_data_perfect"
...
with sync_playwright() as p:
    ...
    print("Step 3: Waiting for Cloudflare Turnstile verification...")
    for sec in range(25):
        body_text = page.locator("body").inner_text()
        body_lower = body_text.lower()
        
        if "success!" in body_lower or "success" in body_lower:
            print(f"✓ Cloudflare Turnstile auto-verified (Success!) after {sec+1} seconds!")
            break
        elif "verify you are human" in body_lower or "verify you are a human" in body_lower:
            print(f"Turnstile requires click (sec {sec+1}). Attempting frame click...")
    ...
    # Step 4: Type email and press Continue
    print("Step 4: Typing email into username field...")
    ...
    # Step 5: Type password and press Continue
    pwd_inp = page.locator("input[name='password']:not(.hide), input[id='password']").first
    print("Step 5: Waiting for password field...")
    try:
    ...
    body_text = page.locator("body").inner_text()
    page.screenshot(path="scratch/perfect_post_password.png")

    # Step 6: MFA or Portal check
    if "Verify your identity" in body_text or "mfa" in page.url.lower() or page.locator("input[name='code']").count() > 0:
        print("Step 6: MFA Verification Code Challenge Screen Detected!")
        ...
        page.screenshot(path="scratch/perfect_mfa_screen.png")
        print("Waiting up to 120s for MFA code in scratch/mfa_code.txt...")
        mfa_code = None
        for _ in range(120):
    ...
    # Step 7: Verify portal home page load & child Byron
    print("Step 7: Verifying portal home page load...")
    page.wait_for_timeout(5000)
    print(f"Final Portal URL: {page.url}")
    body_text = page.locator("body").inner_text()
    page.screenshot(path="scratch/perfect_portal_final.png")
    
    if "Byron" in body_text or "Actions" in body_text or "My Children" in body_text:
        print("🎉 SUCCESS! Confirmed Byron / child profiles on Bright Horizons portal!")
    else:
